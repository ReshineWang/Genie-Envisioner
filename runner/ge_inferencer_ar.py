...
class Inferencer:
    ...
    def validate(
        ...
        for i_validation in range(n_validation):
            ...
            # Autoregressive state variable
            prev_generated_video = None

            full_generated_video_list = []
            full_gt_video_list = []
            ...
            for i_chunk_action in range(n_chunk_action):
                ...
                if i_chunk_action == 0:
                    # First step: Use Ground Truth from batch
                    image = batch['video'][:,:,:,:self.args.data['train']['n_previous']]
                    image = image[:batch_size]
                    image = rearrange(image, 'b c v t h w -> (b v) c t h w')
                else:
                    # Subsequent steps: Use previous generated video
                    # prev_generated_video is (b*v, c, t, h, w)
                    # We take the last n_previous frames
                    n_prev = self.args.data['train']['n_previous']
                    image = prev_generated_video[:, :, -n_prev:]
                    # Ensure it's on the correct device/dtype if needed, though usually it stays on GPU
                # ----------------------------
                ...
                preds = pipe.infer(
                    ...
                    Asyn_infer=getattr(self.args, "Asyn_infer", True),
                )[0]

                # --- Update Autoregressive State ---
                if self.args.return_video and 'video' in preds:
                    # preds['video'] shape: (b*v, c, t, h, w)
                    prev_generated_video = preds['video'].detach()

                    full_generated_video_list.append(prev_generated_video.cpu())
                    full_gt_video_list.append(rearrange(gt_video, 'b c v t h w -> (b v) c t h w').cpu())
                # -----------------------------------
            ...
            # --- Save Full Videos after loop ---
            if self.args.return_video and len(full_generated_video_list) > 0:
                full_gen_video = torch.cat(full_generated_video_list, dim=2)
                full_gt_video = torch.cat(full_gt_video_list, dim=2)
                
                save_cap = f'Validation_Full_{i_validation}'
                fps = (self.args.data['train']['chunk']-1)//self.TEMPORAL_DOWN_RATIO+1
                
                # Separate views and save
                # full_gen_video: (b*v, c, t, h, w) -> (b, v, c, t, h, w)
                gen_views = rearrange(full_gen_video, '(b v) c t h w -> b v c t h w', v=n_view)
                gt_views = rearrange(full_gt_video, '(b v) c t h w -> b v c t h w', v=n_view)
                
                for v_idx in range(n_view):
                    # Save GT
                    save_video(
                        gt_views[0, v_idx], 
                        os.path.join(model_save_dir, f'{save_cap}_view_{v_idx}_gt.mp4'), 
                        fps=15
                    )
                    # Save Generated
                    save_video(
                        gen_views[0, v_idx], 
                        os.path.join(model_save_dir, f'{save_cap}_view_{v_idx}.mp4'), 
                        fps=15
                    )
            # -----------------------------------
            ...
            if self.args.return_action:
                ...
                # Save actions to HDF5
                import h5py
                h5_path = os.path.join(model_save_dir, f'Validation_Full_{i_validation}_actions.h5')
                with h5py.File(h5_path, 'w') as f:
                    f.create_dataset('action', data=pd_actions_arr_all)
                    f.create_dataset('gt_action', data=gt_actions_arr_all)
                print(f"Saved full actions to {h5_path}")
